      ...
      resources: {
        ...
        ru: {
          translation: {
            ...
            // Переводы страницы о переменных
            "variables_title": "Переменные в Bash: Полное руководство",
            "variables_introduction": "Переменные в Bash - это основные строительные блоки, которые помогают хранить и управлять данными в ваших скриптах. От простого текста до сложных массивов, понимание переменных - ключ к освоению Bash-скриптинга.",
            
            "basic_variables_title": "Основы переменных",
            "basic_variables_description": "В Bash переменные хранят данные, которые можно использовать на протяжении всего скрипта. Они могут содержать строки, числа или массивы. Объявление не требуется - переменные создаются при первом присваивании значения.",
            
            "variable_types_title": "Типы переменных",
            "variable_type_1": "Строковые переменные: Хранят текст и символьные данные",
            "variable_type_2": "Целочисленные переменные: Хранят числовые значения",
            "variable_type_3": "Массивы: Хранят несколько значений в одной переменной",
            "variable_type_4": "Константы (readonly): Неизменяемые переменные",
            
            "environment_variables_title": "Переменные окружения",
            "environment_variables_description": "Переменные окружения - это специальные переменные, влияющие на поведение оболочки и программ. Они наследуются всеми процессами, запущенными в сессии оболочки.",
            
            "special_variables_title": "Специальные переменные",
            "special_variables_description": "В Bash есть несколько встроенных специальных переменных, которые предоставляют информацию о скрипте, его аргументах и статусе выполнения.",
            
            "variable_operations_title": "Операции с переменными",
            "variable_operations_description": "Bash предоставляет различные операции для манипуляции переменными, включая строковые операции, арифметические операции и расширения параметров.",
            
            "variables_best_practices_title": "Лучшие практики",
            "variables_best_practice_1": "Всегда заключайте расширения переменных в кавычки для обработки пробелов и специальных символов",
            "variables_best_practice_2": "Используйте осмысленные имена переменных, отражающие их назначение",
            "variables_best_practice_3": "Инициализируйте переменные перед использованием",
            "variables_best_practice_4": "Используйте readonly для констант, чтобы предотвратить случайные изменения",
            
            "common_mistakes_title": "Распространённые ошибки",
            "common_mistake_1": "Забывание пробелов вокруг = при присваивании (неправильно: var=value, правильно: var = value)",
            "common_mistake_2": "Отсутствие кавычек для переменных, содержащих пробелы или специальные символы",
            "common_mistake_3": "Использование переменных до их инициализации",
            "common_mistake_4": "Путаница между локальной и глобальной областью видимости переменных"
          }
        }
      },
